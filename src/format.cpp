...
/// Convert time is seconds to string of format HH:MM:SS
string Format::ElapsedTime(long secs) {
  long hr = secs / (60 * 60);
  long min = (secs - (hr * 60 * 60)) / 60;
  long s = secs - (hr * 60 * 60) - (min * 60);
  // auto new_str = std::string(n_zero - std::min(n_zero, old_str.length()),
  // '0') + old_str;
  ...
  string min_ns =
      string(n_zero - std::min(n_zero, min_s.length()), '0') + min_s;
